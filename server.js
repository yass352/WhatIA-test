...
const INTENTS = [
  ...
  { name: 'prix_disponibilite', priority: 100, handoff: true, tests: [/\b(prix|tarif)s?\b.*\b(disponib|disponibil)\b/i, /\bavez[- ]?vous\s+des\s+places\b/i, /\bprix\s+et\s+disponibilite\b/i, /\bcombien\s+ca\s+coute\b/i, /\bc\s+est\s+combien\b/i, /\bprix\s+billet\b/i, /\bvous\s+avez\s+des\s+tarifs\s+pour\b/i, /\bca\s+me\s+coutera\s+combien\b/i], response: 'Les prix varient selon la destination, la date, la compagnie aérienne et les places disponibles.\nMerci de nous indiquer :\nvotre destination,\nvotre ville de départ et votre ville de retour,\nvos dates de départ et de retour,\nle nombre de passagers,\nvotre préférence éventuelle : compagnie aérienne, vol direct ou prix le plus bas. Un conseiller AMI Voyages prendra ensuite le relais.' },
